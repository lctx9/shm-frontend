...
export default function StaffManagement() {
    ...
    const handleCreateStaff = async (event) => {
        ...
        try {
            ...
            setError(err.message || 'Unable to create the staff account.');
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleStatus = async (userId, status) => {
        const actionText = status === 'REJECTED' ? 'lock' : 'unlock';
        if (!window.confirm(`Are you sure you want to ${actionText} this account?`)) return;

        try {
            setError('');
            await axiosClient.put(`/users/${userId}/status`, { status, reason: status === 'REJECTED' ? 'Account locked by an administrator' : '' });
            await fetchUsers();
        } catch (err) {
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6">
            ...
            <section className="rounded-lg border border-blue-100 bg-white p-8 shadow-sm">
                ...
                <h2 className="mt-2 text-2xl font-black uppercase tracking-wide text-slate-900">Create staff account</h2>
                <p className="mt-2 text-sm text-slate-600">A staff member can be assigned as a mentor in one track and as a judge in another round.</p>

                <form onSubmit={handleCreateStaff} className="mt-6 grid gap-4 md:grid-cols-[1fr_1fr_160px_160px_auto]">
                    <input required className="input-custom" value={staffForm.fullName} onChange={(e) => setStaffForm({ ...staffForm, fullName: e.target.value })} placeholder="Full name" />
                    ...
                    <div className="relative">
                        <input required minLength="6" type={showPassword ? 'text' : 'password'} className="input-custom w-full pr-10" value={staffForm.password} onChange={(e) => setStaffForm({ ...staffForm, password: e.target.value })} placeholder="Initial password" />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-600" title={showPassword ? 'Hide password' : 'Show password'}>
                            {showPassword ? (
            ...
            <section className="rounded-lg border border-blue-100 bg-white shadow-sm">
                <div className="flex flex-col justify-between gap-4 border-b border-blue-100 bg-blue-50 px-6 py-4 sm:flex-row sm:items-end">
                    <div>
                        <h2 className="text-xl font-black uppercase tracking-wide text-slate-900">Staff list</h2>
                        <p className="mt-1 text-sm text-slate-600">Mentor and judge duties are assignments, not account roles.</p>
                    </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-blue-50">
                            {loading ? (
                                ...
                                <tr><td colSpan="4" className="px-6 py-8 text-center text-slate-500">No matching staff accounts.</td></tr>
                            ) : staffUsers.map((user) => (
                                <tr key={user.id} className="hover:bg-blue-50/40">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2">
                                            {user.status !== 'APPROVED' && <button type="button" onClick={() => handleStatus(user.id, 'APPROVED')} className="btn-primary">Unlock</button>}
                                            {user.status === 'APPROVED' && <button type="button" onClick={() => handleStatus(user.id, 'REJECTED')} className="btn-secondary">Lock</button>}
                                        </div>
                                    </td>
                                </tr>
                            ))}
