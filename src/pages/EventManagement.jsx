...
export default function EventManagement() {
    ...
    if (!form.id && !showCreate) {
        return (
            <div className="mx-auto max-w-7xl space-y-6">
                <section className="overflow-hidden rounded-2xl border border-[#d7e6f8] bg-white p-6 shadow-sm md:p-8">
                    <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                        <div>
                            <p className="text-xs font-black uppercase tracking-[0.2em] text-[#0f63c9]">Event operations</p>
                            <h2 className="mt-2 text-2xl font-black text-[#071936] md:text-3xl">Tổng quan sự kiện</h2>
                            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#5c6d83]">Theo dõi toàn bộ mùa giải, tiến độ cấu hình và trạng thái vận hành tại một nơi.</p>
                        </div>
                        <button type="button" className="btn-primary" onClick={() => { setForm(emptyEvent()); setShowCreate(true); setCreateStep(0); setMessage(null); }}>+ Tạo sự kiện mới</button>
                    </div>
                </section>
    ...
    if (!form.id) {
        ...
        return (
            <div className="mx-auto max-w-6xl pb-8">
                    ...
                    <div className="overflow-hidden rounded-2xl border border-[#d7e6f8] bg-white shadow-sm">
                    <header className="border-b border-[#d7e6f8] bg-[#f8fbff] px-6 py-7 md:px-10">
                        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                            <div>
                                <p className="text-xs font-black uppercase tracking-[0.22em] text-[#0f63c9]">Coordinator · Tạo cuộc thi</p>
                                <h1 className="mt-2 text-2xl font-black text-[#071936] md:text-3xl">Thiết lập cuộc thi mới</h1>
                                <p className="mt-2 max-w-2xl text-sm text-[#5c6d83]">Đi từng bước. Hệ thống sẽ tự sinh bảng, vòng loại và một trận chung kết.</p>
                            </div>
                            <button
                                type="button"
                    ...
                    <div className="min-h-[520px] p-6 md:p-10">
                        ...
                        {createStep === 2 && (
                            <div className="space-y-6">
                                ...
                                <div className="rounded-xl border border-[#d7e6f8] bg-[#f0f7ff] p-5 text-[#071936]">
                                    <p className="text-xs font-black uppercase tracking-[0.18em] text-[#0f63c9]">Xem trước cấu trúc</p>
                                    <p className="mt-2 text-xl font-black text-[#071936]">{form.tracks.length} bảng × {Number(form.roundCount) - 1} vòng loại + 1 chung kết = {matchCount} vòng đấu</p>
                                    <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-[#0f63c9]">
                                        {form.tracks.map((track) => <span key={track.name} className="rounded-full bg-white border border-[#d7e6f8] px-3 py-1.5 shadow-sm">{track.name || 'Chưa đặt tên'}</span>)}
                                        <span className="rounded-full bg-emerald-50 border border-emerald-200 px-3 py-1.5 text-emerald-700">Chung kết</span>
                                    </div>
                                </div>
                            </div>
                        )}
    ...
    return (
        <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="space-y-6">
                <section className="overflow-hidden rounded-2xl border border-[#d7e6f8] bg-white p-6 shadow-sm md:p-8">
                    <button type="button" className="text-xs font-black text-[#0f63c9] hover:underline" onClick={() => { setForm(emptyEvent()); setSelectedEventId(''); setShowCreate(false); setMessage(null); }}>← Quay lại danh sách sự kiện</button>
                    <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <div className="flex flex-wrap items-center gap-2">
                                <span className="rounded-full bg-[#f0f7ff] border border-[#d7e6f8] px-3 py-1 text-xs font-black text-[#0f63c9]">{selectedEvent?.season} {selectedEvent?.year}</span>
                            ...
                            <p className="mt-2 text-sm text-[#5c6d83]">{selectedEvent?.teamCount || 0} đội thi · {selectedEvent?.tracks?.length || 0} bảng đấu · {selectedEvent?.roundCount || 0} vòng</p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            <Link to={`/events/${selectedEventId}`} className="btn-secondary">Xem trang công khai</Link>
                            <Link to="/dashboard/scoring-config" className="btn-primary">Cấu hình chấm điểm</Link>
                        </div>
                    </div>
                </section>
