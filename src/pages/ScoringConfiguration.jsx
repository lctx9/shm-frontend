...
const emptyForm = () => ({ guidelineUrl: '', submissionStartDate: '', submissionDeadline: '', durationMinutes: 60, gradingDurationMinutes: 10, topN: 10, judgeIds: [], criteria: defaultCriteria });
...
function formFromMatrix(matrix) {
    ...
    return matrix ? {
        ...
        gradingDurationMinutes: matrix.gradingDurationMinutes || 10,
        topN: matrix.topN ?? '',
...
export default function ScoringConfiguration() {
    ...
    const payloadFor = (matrix) => ({
        ...
        gradingDurationMinutes: Number(form.gradingDurationMinutes) || 10,
                    ...
                    <main className="min-w-0 space-y-5">
                        ...
                        {activeStep === 'setup' && <section className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm">
                            ...
                            <div className="mt-5 grid gap-4 md:grid-cols-3">
                                ...
                                <label className="text-sm font-bold text-slate-700">Grading duration (Minutes)<input type="number" min="1" className="input-custom mt-2 font-bold text-emerald-700" placeholder="VD: 10, 15, 30" value={form.gradingDurationMinutes} onChange={(event) => setForm({ ...form, gradingDurationMinutes: event.target.value })} /></label>
                            </div>
                            <div className="mt-4 rounded-xl border border-blue-100 bg-blue-50/70 p-3.5 text-xs text-blue-900 leading-relaxed font-medium">
                                ⏱️ <strong>Time mechanism:</strong> Competition round test duration is <strong>{form.durationMinutes || 60} minutes</strong>. Grading time countdown is set to <strong>{form.gradingDurationMinutes || 10} minutes</strong>.
                                {Number(selectedMatrix?.roundOrder) === 1 ? (
                                    <span> Round 1 will automatically count down {form.durationMinutes || 60} minutes starting from tournament start time, followed by <strong>{form.gradingDurationMinutes || 10} minutes</strong> for judges to submit scores.</span>
                                ) : (
                                    <span> Subsequent rounds will count down {form.durationMinutes || 60} minutes after the Coordinator opens the round, followed by <strong>{form.gradingDurationMinutes || 10} minutes</strong> for judges to submit scores.</span>
                                )}
                            </div>
